...
class AppFacade {
 ...
 public:
  ...
  void initialize() {
    ...
    // set up strategy (uniform strategy is default, although configuration
    // parser throws on invalid policy)
    if ((config.policy == "uniform") || config.policy.empty()) {
      ...
      strategy =
          std::make_unique<WeightedStrategy>(config.directory_priorities);
      context.set_strategy(std::move(strategy));
    }
  };
  ...
  std::vector<std::filesystem::path> get_dirfiles() {
    Directory root_dir{config.path, config.directory_exclusions,
                       config.file_allowed_extensions};
  ...
  ChangeParameters change_wallpaper() {
    ...
    // set up command
    ChangeCommand command{wallpaper_path, config.directory_seconds,
   ...
   */
  void log(const std::string &command_line) { logger.log(command_line); }
};