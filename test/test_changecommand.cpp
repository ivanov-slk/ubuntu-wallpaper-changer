...
TEST(ChangeCommandTests, ReturnsSeconds) {
  std::vector<std::pair<std::string, int>> change_seconds_vector{
      {"dir1", 10}, {"dir2", 20}, {"exist", 5}};
  std::map<std::string, std::string> dir_pic_options;
  ChangeCommand testable{"/this/path/does/not/exist/1 (2).jpg",
                         change_seconds_vector, 30, dir_pic_options};
  testable.execute();
  ChangeParameters result = testable.get_change();
...
TEST(ChangeCommandTests, ReturnsPictureOptions) {
  ...
  testable.execute();
  ChangeParameters result = testable.get_change();
  ASSERT_EQ("gsettings set org.gnome.desktop.background picture-options scaled",
            result.command_line_options);
}