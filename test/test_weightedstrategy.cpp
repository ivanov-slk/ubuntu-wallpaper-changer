...
TEST(WeightedStrategyTests, SubdirsOverlap) {
  ...
  std::vector<std::filesystem::path> test_input{"/path/dir2", "/path/other_dir",
                                                "/path/dir3"};
...
TEST(WeightedStrategyTests, SubdirsPrioritiesEmpty) {
  std::vector<std::pair<std::filesystem::path, int>> input_dir_priorities;
  std::vector<std::filesystem::path> test_input{"other_dir1", "other_dir2",
                                                "other_dir3"};
  std::vector<std::pair<std::filesystem::path, int>> correct{
...
TEST(WeightedStrategyTests, NormalizationNonEmpty) {
  ...
  std::vector<std::pair<std::filesystem::path, float>> result =
      testable.normalize_priorities(test_input);
  ASSERT_TRUE(result == correct);
}
...
TEST(WeightedStrategyTests, PickDirectoryPriorities1) {
  ...
  std::filesystem::path result =
      testable.pick_directory(normalized_priorities, true);
  ASSERT_TRUE(result == correct);
}
...
TEST(WeightedStrategyTests, ReturnsAllFiles) {
  ...
  std::vector<std::filesystem::path> result =
      testable.execute(Directory("../test/resources/dir_tests/test_dir3"));
  ...
  std::sort(correct2.begin(), correct2.end(),
            std::greater<std::filesystem::path>());
  std::sort(correct3.begin(), correct3.end(),
            std::greater<std::filesystem::path>());
  ASSERT_TRUE((result == correct1) || (result == correct2) ||
              (result == correct3));
}
...
TEST(WeightedStrategyTests, ConfirmProportions) {
  ...
  ASSERT_TRUE((result_test_dir3.size() <= 800) &&
              (result_test_dir3.size() >= 600));
